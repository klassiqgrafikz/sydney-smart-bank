...
import { useTransferRestriction } from "@/hooks/use-transfer-restriction";

export function restrictionMessage(restoreLabel: string) {
  return `Dear customer you can't make transfers at this moment as a new user, kindly hold patience while we restore your services! Restore date: ${restoreLabel}.`;
}
    ...
    <>
      {restriction && (
        <div className="mb-4 flex gap-2 rounded-lg border border-destructive/50 bg-destructive/5 p-4 text-sm text-destructive">
          ...
          <div>{restrictionMessage(restriction.restoreLabel)}</div>
        </div>
      )}
      <div
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="pt-2 text-foreground">
              {restriction
                ? restrictionMessage(restriction.restoreLabel)
