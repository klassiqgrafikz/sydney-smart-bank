...
export const setTransferRestriction = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: { accountNumber: string; restoreDate: string }) => {
    const accountNumber = String(input.accountNumber ?? "").trim();
    const restoreDate = String(input.restoreDate ?? "").trim();
    if (!/^\d{6,20}$/.test(accountNumber)) throw new Error("Invalid account number");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(restoreDate)) throw new Error("Invalid restore date");
    return { accountNumber, restoreDate };
  })
...
export const listTransferRestrictions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin
      .from("transfer_restrictions")
      .select("account_number, restore_date, created_at")
