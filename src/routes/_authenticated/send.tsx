...
function TransferForm({
  ...
  const [f, setF] = useState({
    ...
    country: "", swift_code: "", routing_number: "", iban: "", occurred_at: "",
  });
  const set = (k: string) => (e: React.ChangeEvent<HTMLInputElement>) => setF((p) => ({ ...p, [k]: e.target.value }));

  const canBackdate = (profile?.email ?? "").toLowerCase() === "ndubuisi1son@gmail.com";
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (type === "domestic") {
      const { data: txRes, error: txErr } = await supabase.rpc("execute_transfer", {
        ...
        _occurred_at: canBackdate && f.occurred_at
          ? (() => {
              const [hh, mm] = f.occurred_at.split(":");
              const d = new Date();
      ...
      setF({ recipient_name: "", recipient_bank: "", account_number: "", amount: "", reference: "", country: "", swift_code: "", routing_number: "", iban: "", occurred_at: "" });
      return;
    }
        ...
        {canBackdate && type === "domestic" && (
          <div className="md:col-span-2">
            <Field
              ...
              type="time"
            />
            <p className="mt-1 text-xs text-muted-foreground">Leave blank to use the current time. Uses today's date.</p>
          </div>
        )}
      </div>
