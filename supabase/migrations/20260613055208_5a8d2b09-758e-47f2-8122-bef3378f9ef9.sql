...
BEGIN
  ...
  INSERT INTO public.transactions (user_id, sender_name, receiver_name, amount, transaction_type, description, status)
  VALUES (target_id, _sender_name, coalesce(nullif(target_name,''),'Account holder'), _amount, 'credit', 'Admin deposit', 'completed')
...
REVOKE ALL ON FUNCTION public.admin_fund_account_by_number(uuid, text, numeric, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_fund_account_by_number(uuid, text, numeric, text) TO service_role;
